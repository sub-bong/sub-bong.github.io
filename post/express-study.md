...
| storage 속성       |                                                                                                                                                                                                                                    |
| :----------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `destination` 함수 | **어디에 저장할 것인지** `req` 매개변수: 요청에 대한 정보 `file(객체)` 매개변수: 업로드한 파일에 대한 정보를 포함 `done(함수)`: **첫 번째 인수**에는 에러가 있다면 설정 가능, **두 번째 인수**는 실제 경로나 파일 이름 넣기        |
| `filename`함수     | **어떤 이름으로 저장할 것인지** `req` 매개변수: 요청에 대한 정보 `file(객체)` 매개변수: 업로드한 파일에 대한 정보를 포함 `done(함수)`: **첫 번째 인수**에는 에러가 있다면 설정 가능, **두 번째 인수**는 실제 경로나 파일 이름 넣기 |
...
- URL : `localhost:3000`/`app.js의 app.use('/주소')`/`router.get('/주소')`
- 코드 : `app.use("/about", aboutRouter);`
